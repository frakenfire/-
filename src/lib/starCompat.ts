...
const ARCHETYPE: Record<CompatVibe, string[]> = {
  twin: ['같은 별에서 온 단짝', '너무 닮은 사이', '닮은꼴 짝'],
  harmony: ZODIAC_ARCHETYPE.harmony,
  steady: ['꾸준히 잘 맞는 조합', '그날 분위기로 승부하는 사이', '무난하게 잘 맞는 별자리'],
...
const REASON: Record<Tag, string[]> = {
  ...
  trine: [
    '같은 원소 별자리라 성격이 비슷해요. 자연스럽게 통하는 조합이에요.',
    '원소가 같은 별자리 조합이에요. 점성술에서 궁합이 좋다고 보는 조합이에요.',
  ],
  compatible: [
    '불과 바람, 흙과 물처럼 서로 다르지만 잘 통하는 원소 조합이에요.',
    '서로 다른 원소지만 잘 맞기로 꼽히는 조합이에요.',
  ],
  opposite: [
    '별자리 상 정반대편에 있는 사이예요. 다르니까 오히려 끌리는 조합이에요.',
    '정반대 별자리라 매력도 정반대예요. 그게 은근 잘 맞아요.',
  ],
  friction: [
    '원소끼리 부딪히는 조합이라 팽팽한 데가 있어요. 근데 그만큼 서로를 자극하는 사이예요.',
    '서로 다른 성격의 원소라 말을 주고받다 보면 세질 수 있는 조합이에요.',
  ],
  neutral: [
    '뚜렷하게 정해진 상성은 아니에요. 그날그날 분위기로 승부를 보는 조합이에요.',
    '원소상 특별히 얽힌 관계는 아니라, 그날그날 분위기가 갈리는 편이에요.',
  ],
};

const HEAD_BEST = [
  '오늘은 별자리 궁합이 크게 겹치는 날이에요.',
  '이 조합, 오늘따라 텔레파시가 통해요.',
  '오늘 하루 서로에게 든든한 별이에요.',
