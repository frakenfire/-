...
const HEADLINE: Record<SajuTone, string[]> = {
  great: [
    '오늘 날짜와 내 띠가 잘 맞아서, 일이 크게 열리는 날이에요.',
    '오늘은 하는 일이 내 편인 날이에요. 미뤄둔 걸 꺼내기 좋아요.',
    '오늘은 손발이 잘 맞아, 하려던 일에 힘이 실려요.',
    '오늘은 무슨 일이든 척척 맞아떨어져서 수월하게 끝나요.',
    '막혀 있던 게 활짝 열리는 날이에요. 한 걸음 크게 내딛기 좋아요.',
    '오늘은 주변이 나를 밀어주는 날이라, 자신 있게 움직여도 돼요.',
  ],
  good: [
    '오늘 날짜와 내 띠가 무난히 어울려서, 순조로운 날이에요.',
    '큰 굴곡 없이 부드럽게 지나가는 하루예요.',
    '오늘은 손발이 맞는 편이라 편하게 풀려요.',
    '오늘은 억지로 애쓰지 않아도 술술 이어지는 날이에요.',
    '조용히 좋은 쪽으로 풀려가는 날이에요.',
    '오늘은 작은 일들이 기분 좋게 맞물리는 날이에요.',
  ],
  steady: [
    '오늘은 부딪힐 일 없이 조용히 지나가는 날이에요.',
    '오늘은 마음이 평온해요. 익숙한 것에서 힘이 나와요.',
    '무리 없이 늘 하던 대로 가면 좋은 날이에요.',
    '오늘은 특별한 일 없이 평온하게 지나가는 하루예요.',
    '들뜨기보다 차분히 나를 챙기기 좋은 날이에요.',
    '오늘은 평소 속도만 지키면 무난한 날이에요.',
  ],
  caution: [
    '오늘 날짜와 내 띠가 부딪히는 편이라, 한 박자 늦추면 좋아요.',
    '신경이 팽팽해지는 날이에요. 욱하는 순간만 피하면 무난해요.',
    '오늘은 밀어붙이기보다 지키는 쪽이 이득이에요.',
    '오늘은 일정이 살짝 어긋나기 쉬우니, 여유를 두면 좋아요.',
    '서로 엇갈리기 쉬운 날이에요. 급한 마음만 내려두면 괜찮아요.',
...
export const TIP: Record<SajuTone, string[]> = {
  ...
  good: [
    '가볍게 시작한 일이 잘 풀려요.',
    '사람 만나는 자리에서 좋은 일이 생겨요.',
    '오늘은 부탁이나 제안을 해보기 좋아요.',
    '작은 약속 하나를 지키면 하루가 더 잘 풀려요.',
