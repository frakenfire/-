...
export const UNSEONG_KO: Record<Unseong, { word: string; line: string }> = {
  ...
  건록: { word: '제 몫 하는 자리', line: '스스로 버티는 힘이 제일 안정적인 자리예요.' },
  제왕: { word: '가장 센 자리', line: '힘이 꼭대기에 있는 자리예요. 밀어붙이면 통하지만 꺾이면 크게 꺾여요.' },
...
// 셋이 다 모여야 형으로 보는 관법(triple)도 널리 쓰인다. 어느 쪽인지 규칙 파일에
// 적어뒀고, 바꾸면 형이 뜨던 사람들의 결과가 달라지므로 값은 그대로 둔다.
...
export const SINSAL_KO: Record<SinsalKey, { word: string; line: string }> = {
  ...
  도화: { word: '눈에 띄는 기운', line: '사람 눈에 잘 띄는 자리예요. 인기와 구설이 같이 와요.' },
  역마: { word: '움직이는 기운', line: '자리를 옮기고 멀리 가는 자리예요. 한곳에 오래 못 있어요.' },
  화개: { word: '혼자 깊어지는 기운', line: '파고들고 만들어내는 자리예요. 사람보다 혼자가 편해요.' },
};
