...
const RULES = [
  ...
  [/복병|자동화|피드백|반영해|확보해|정의하는|미션|재테크|이벤트|리듬 관리|설계예요|인지해|전략이에요|스위치|리액션|적중률이|배치해요|전달해요|배가되|마중물|연료가|연료로|패스해도|소비 패턴|최대의|무드예요|채택돼|방향 감각|의 감각|하루의 온도|재생을 멈추|워밍업|잠의 질|능률|검토해요|배치하는|명단|페이스만|리듬이 잡|리듬부터|내 리듬|진폭|회수|관건|검증|소진|핵심이에요|의 축|시기의 성질|정의하|소모|통제[하 ]|불확실|관리가 돼요|산출물|전문성|손실|피드를|확보하기|손익|멀티태스킹|인풋|컨트롤|판명|재검토|시뮬레이션|관리 대상|통계적|창의력|의 지분|마음의 체력|하루 리듬|의 균형을/,
   "회사 문서 말고 사람이 하는 말로"],
];
